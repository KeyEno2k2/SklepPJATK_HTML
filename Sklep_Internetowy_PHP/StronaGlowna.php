...
<head>
    ...
    <script> /Using native JavaScript/ var element = document.querySelector(".typ"); element.onclick = function(event) {   event.preventDefault();   document.querySelector("#form").submit(); }; </script>
</head>
<style>
    body {
        font-family: "Arial";
...
<!----------------------------------------------------------------------------------------------->
<div id="Topnav2">
    <center><h1>Gorący Produkt!!!</h1></center>
    <p></p>
    <?php
    $_SESSION['Laptop'] = 25;
    ...
    if ($result -> num_rows > 0 )
        $piec = $result -> fetch_assoc();
    $_SESSION['Produkt'] = $piec['Nazwa'];
    $_SESSION['Produkt_TYP'] = $piec['Typ_ID'];
    echo "<center><h2>".$piec['Nazwa']."</h2></center>";
    echo "<center><a href='Produkt.php'><img src=".'"'.$piec['Zdjecie_URL'].'">'."</img></a></center>";
    $conn -> close();
    ?>
</div>

<center><section id="Galeria" style="margin-top: 30px">
