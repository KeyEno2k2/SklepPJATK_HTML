...
<!----------------------------------------------------------------------------------------------->
<div id="TAPETA" style="max-width: 100%; margin-left: -20px">
<?php
include ("Database.php");
$db = new Database();
$sql = "SELECT opinie.ID as ID, Opinia, produkty.Nazwa as Nazwa FROM opinie inner join produkty on produkty.ID = Produkty_ID";
$result = $db -> get($sql);
?>
<table class="table table-striped table-dark" style="margin-right: 15px; margin-left: 15px">
    ...
    <?php
        ...
        while ($row = $result -> fetch_assoc()){
            echo "<tr><td>".$row['Opinia']."</td><td>".$row['Nazwa']."</td><td><form action='UsunOpinie.php' method='post' ><input name='DELETE' type='hidden' value='".$row['ID']."'><input type='submit' class='btn btn-danger' value='USUŃ'></form></td></tr>";
        }
?>
    </tbody>
</table>
